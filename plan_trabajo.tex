...
\subsection*{Roles}
\begin{itemize}
    \item \textbf{Integrante A} — Backend + Integración (SPICE, simulación, API, validación de circuitos)
    \item \textbf{Integrante B} — Frontend núcleo (editor, componentes, conexiones, store, hooks)
    \item \textbf{Integrante C} — Frontend UI/UX (toolbar, paneles, calculadora, gráficas, dark mode, traducción)
\end{itemize}
...
\begin{itemize}
    \item El build de producción se completa sin errores (\texttt{npm run build}).

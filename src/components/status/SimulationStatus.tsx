...
export function SimulationStatus() {
  ...
  const status = !simulationRunning ? 'stopped'
    : simError ? 'error'
    : simResults?.status.success ? 'running'
    : 'processing';

  const ledColor = status === 'running' ? 'bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.6)]'
    : status === 'error' ? 'bg-red-500 shadow-[0_0_6px_rgba(239,68,68,0.6)]'
    : status === 'processing' ? 'bg-yellow-400 shadow-[0_0_6px_rgba(234,179,8,0.6)]'
    : 'bg-slate-600';
  ...
  const nodeCount = simResults?.status.success ? Object.keys(simResults.nodeVoltages).length - 1 : 0;
  const branchCount = simResults?.status.success ? Object.keys(simResults.branchCurrents).length : 0;

  return (
    <div className="flex items-center gap-2 text-[10px]">
      <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-full border transition-all ${
        status === 'running' ? 'bg-green-900/30 border-green-700/50 text-green-300'
        : status === 'error' ? 'bg-red-900/30 border-red-700/50 text-red-300'
        : status === 'processing' ? 'bg-yellow-900/30 border-yellow-700/50 text-yellow-300'
      ...
      {status === 'running' && (
        <span className="text-green-400 font-mono font-medium">{simTime.toFixed(2)}s</span>
      )}

      {status === 'error' && simError && (
        <span className="text-red-400 font-mono truncate max-w-[180px]" title={simError}>
          {simError}
        </span>
      )}
      ...
      {simulationRunning && !simError && simResults?.status.success && (
        <div className="w-16 h-1 bg-surface-800 rounded-full overflow-hidden hidden lg:block">
          <div className="h-full bg-gradient-to-r from-green-500 to-green-400 rounded-full animate-pulse" style={{ width: '100%' }} />
        </div>
      )}

      {groundCount === 0 && compCount > 0 && (
        <span className="text-amber-400 text-[9px] font-medium hidden sm:inline">¡Falta tierra!</span>
      )}
    </div>
  );
}

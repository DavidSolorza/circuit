...
export const TerminalDot: React.FC<{ x: number; y: number; color?: string; label?: string }> = ({ x, y, color = '#3b82f6', label }) => (
  <g>
    <circle cx={x} cy={y} r={6} fill="#0f1019" stroke={color} strokeWidth={2} className="transition-all duration-150" />
    <circle cx={x} cy={y} r={3} fill={color} className="transition-all duration-150" />
    {label && (
      <text x={x + (x < 0 ? -8 : 8)} y={y + 4} fill={color} fontSize={8} fontWeight="bold" textAnchor={x < 0 ? 'end' : 'start'}>
        {label}
      </text>
    )}
  </g>
...
export const ResistorSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.resistor, highlight }) => {
  const seg = 6;
  const sw = size / seg;
  const amp = size * 0.2;
  const left = -size / 2;
...
export const CapacitorSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.capacitor, highlight }) => {
  const g = 10;
  const left = -size / 2;
  const right = size / 2;
  const hh = size * 0.22;
  ...
  return (
    <svg width={size} height={size * 0.5} viewBox={`${left} ${-size * 0.25} ${size} ${size * 0.5}`}>
      ...
      <line x1={-g / 2} y1={-hh} x2={-g / 2} y2={hh} stroke={hc} strokeWidth={3} strokeLinecap="round" />
      <line x1={g / 2} y1={-hh} x2={g / 2} y2={hh} stroke={hc} strokeWidth={3} strokeLinecap="round" />
...
export const InductorSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.inductor, highlight }) => {
  const loops = 4;
...
export const VoltageSourceSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.voltageSource, highlight }) => {
  const r = size * 0.28;
  const cx = 0;
  const cy = 0;
  const left = -size / 2;
  ...
  return (
    <svg width={size} height={size * 0.55} viewBox={`${left} ${-size * 0.28} ${size} ${size * 0.55}`}>
      ...
      <line x1={cx - r * 0.35} y1={cy} x2={cx + r * 0.35} y2={cy} stroke={hc} strokeWidth={2.5} strokeLinecap="round" />
      <line x1={cx} y1={cy - r * 0.35} x2={cx} y2={cy + r * 0.35} stroke={hc} strokeWidth={2.5} strokeLinecap="round" />
...
export const CurrentSourceSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.currentSource, highlight }) => {
  const r = size * 0.28;
  const cx = 0;
  const cy = 0;
  const left = -size / 2;
  ...
  return (
    <svg width={size} height={size * 0.55} viewBox={`${left} ${-size * 0.28} ${size} ${size * 0.55}`}>
      ...
      <line x1={cx + r} y1={cy} x2={right} y2={cy} {...sp(hc)} />
      <circle cx={cx} cy={cy} r={r} {...sp(hc, 2)} />
      <polyline points={`${cx - r * 0.3},${cy + r * 0.35} ${cx + r * 0.3},${cy} ${cx - r * 0.3},${cy - r * 0.35}`} {...sp(hc, 2.5)} />
...
export const LedSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.led, highlight }) => {
  const s = size * 0.25;
  ...
  return (
    <svg width={size} height={size * 0.5} viewBox={`${left} ${-size * 0.25} ${size} ${size * 0.5}`}>
      ...
      <line x1={-bodyW} y1={0} x2={bodyW * 0.5} y2={s} {...sp(hc, 2.5)} />
      <line x1={bodyW * 0.5} y1={-s} x2={bodyW * 0.5} y2={s} stroke={hc} strokeWidth={2.5} strokeLinecap="round" />
      <line x1={bodyW * 0.5} y1={0} x2={right} y2={0} {...sp(hc)} />
      <polygon points={`${-bodyW + 4},${-s + 4} ${-bodyW + 8},${-s + 4} ${-bodyW + 6},${-s - 2}`} fill={hc} />
      <TerminalDot x={left} y={0} color={hc} />
      <TerminalDot x={right} y={0} color={hc} />
    </svg>
  );
...
export const TransistorSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.transistor, highlight }) => {
  const hh = size * 0.22;
  const left = -size / 2;
  const right = size / 2;
  const hc = highlight ? '#60a5fa' : color;
...
export const SwitchSvg: React.FC<{ size?: number; color?: string; highlight?: boolean; closed?: boolean }> = ({ size = 80, color = colors.switch, highlight, closed = false }) => {
  const left = -size / 2;
  const right = size / 2;
  const hc = highlight ? '#60a5fa' : color;
  return (
    <svg width={size} height={size * 0.5} viewBox={`${left} ${-size * 0.25} ${size} ${size * 0.5}`}>
      {closed ? (
        <line x1={left} y1={0} x2={right} y2={0} stroke={hc} strokeWidth={3} strokeLinecap="round" />
      ) : (
        <>
...
export const GroundSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.ground, highlight }) => {
  const w = size * 0.4;
  const hc = highlight ? '#60a5fa' : color;
  return (
    <svg width={size} height={size * 0.5} viewBox={`${-size / 2} ${-size * 0.25} ${size} ${size * 0.5}`}>
      ...
      <line x1={-w * 0.7} y1={6} x2={w * 0.7} y2={6} stroke={hc} strokeWidth={2} strokeLinecap="round" />
      <line x1={-w * 0.4} y1={12} x2={w * 0.4} y2={12} stroke={hc} strokeWidth={1.5} strokeLinecap="round" />
    </svg>
  );
};

export const VoltmeterSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.voltmeter, highlight }) => {
  const r = size * 0.28;
  const cx = 0;
  const cy = 0;
  const left = -size / 2;
  ...
  return (
    <svg width={size} height={size * 0.55} viewBox={`${left} ${-size * 0.28} ${size} ${size * 0.55}`}>
      ...
      <text x={cx} y={cy + 4.5} fill={hc} fontSize={13} fontWeight="bold" textAnchor="middle" fontFamily="serif">V</text>
      <TerminalDot x={left} y={cy} color={hc} />
      <TerminalDot x={right} y={cy} color={hc} />
    </svg>
  );
...
export const AmmeterSvg: React.FC<{ size?: number; color?: string; highlight?: boolean }> = ({ size = 80, color = colors.ammeter, highlight }) => {
  const r = size * 0.28;
  const cx = 0;
  const cy = 0;
  const left = -size / 2;
  ...
  return (
    <svg width={size} height={size * 0.55} viewBox={`${left} ${-size * 0.28} ${size} ${size * 0.55}`}>
      ...
      <line x1={cx + r} y1={cy} x2={right} y2={cy} {...sp(hc)} />
      <circle cx={cx} cy={cy} r={r} {...sp(hc, 2)} fill={hc} fillOpacity={0.1} />
      <text x={cx} y={cy + 4.5} fill={hc} fontSize={13} fontWeight="bold" textAnchor="middle" fontFamily="serif">A</text>
      <TerminalDot x={left} y={cy} color={hc} />
      <TerminalDot x={right} y={cy} color={hc} />
    </svg>
  );
...
export const symbolComponents: Record<string, React.FC<{ size?: number; color?: string; highlight?: boolean; closed?: boolean }>> = {
